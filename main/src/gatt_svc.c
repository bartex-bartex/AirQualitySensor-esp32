...
static void user_svc_access(uint16_t conn_handle, uint16_t attr_handle,
                          struct ble_gatt_access_ctxt *ctxt, void *arg);
...
/* User service */
static const ble_uuid16_t user_svc_uuid = 
    BLE_UUID128_INIT(0x23, 0xd1, 0xbc, 0xea, 0x5f, 0x78, 0x23, 0x15, 0xde, 0xef,
                     0x12, 0x12, 0x25, 0x15, 0x00, 0x06);

static uint16_t user_chr_val_handle;
static const ble_uuid128_t user_chr_uuid =
    BLE_UUID128_INIT(0x23, 0xd1, 0xbc, 0xea, 0x5f, 0x78, 0x23, 0x15, 0xde, 0xef,
                     0x12, 0x12, 0x25, 0x15, 0x00, 0x07);
...
static const struct ble_gatt_svc_def gatt_svr_svcs[] = {
    ...
    {
        .type = BLE_GATT_SVC_TYPE_PRIMARY,
        .uuid = &user_svc_uuid.u,
        .characteristics =
            (struct ble_gatt_chr_def[])
            {
                {/* User characteristic */
                    .uuid = &user_chr_uuid.u,
                    .access_cb = user_svc_access,
                    .flags = BLE_GATT_CHR_F_WRITE,
                    .val_handle = &user_chr_val_handle},
                {
                    0 /* No more characteristics. */
                }
            }
...
static void user_svc_access(uint16_t conn_handle, uint16_t attr_handle,
    ...
    case BLE_GATT_ACCESS_OP_WRITE_CHR:
        /* Verify connection handle */
        if (conn_handle != BLE_HS_CONN_HANDLE_NONE) {
            ESP_LOGI(TAG, "User write; conn_handle=%d, attr_handle=%d", conn_handle, attr_handle);
        } else {
            ESP_LOGI(TAG, "Characteristic write by nimble stack with no connection; attr_handle=%d", attr_handle);
        }

        if (attr_handle == user_chr_val_handle) {
            // Allocate a buffer to hold the incoming data
            ESP_LOGI(TAG, "Received data length: %d", ctxt->om->om_len);
            ...
            ESP_LOGI(TAG, "Received (User ID) data: %s", buffer);

            config_user_id_save(buffer);
